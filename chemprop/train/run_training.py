...
from chemprop.utils import build_optimizer, build_lr_scheduler, get_loss_func, load_checkpoint, makedirs, \
    save_checkpoint, save_smiles_splits, fits_to_conds
...
def run_training(args: TrainArgs,
    ...
    #dont scale if doing parameter fitting....
    if args.dataset_type == 'regression' and args.arr_vtf==None:
    ...
    if args.dataset_type == 'multiclass':
        ...
        if args.arr_vtf=='arr':
            sum_test_preds = np.zeros((len(test_smiles), 2))
        elif args.arr_vtf=='vtf':
            sum_test_preds = np.zeros((len(test_smiles), 3))
        else:
    ...
    for model_idx in range(args.ensemble_size):
        ...
        for epoch in trange(args.epochs):
            ...
            ###currently working here to get model to train using multiple output nodes for 
            n_iter = train(
                model=model,
        ...
        if args.arr_vtf is not None:
            test_temps = test_data_loader.dataset.temps()
            test_preds = fits_to_conds(test_preds,test_temps,args.arr_vtf)
