...
def arr_conductivity(logA,Ea,temp):
    ...
    if type(cond) is np.ndarray:
        return np.expand_dims(cond, axis=1)
    else:
...
def fits_to_conds(preds,temps,arr_vtf):
    """
    ...
    if torch.is_tensor(preds):
        temps = torch.FloatTensor(temps).to(preds.device)
    elif type(preds) is np.ndarray:
        temps = np.array(temps)
        
    #run fit parameters with temps through equations to get conductivity values         
    if arr_vtf=='arr':
        conds = arr_conductivity(logA, Ea, temps)
    if arr_vtf=='vtf':
        conds = vtf_conductivity(logA, Ea, T0, temps)
    
    return conds

def arrhenius_reg(logA, Ea):
    """
